...
func main() {
	js.Global().Set("ShowImageAsBase64", js.FuncOf(ShowImageAsBase64))
...
func ShowImageAsBase64(this js.Value, input []js.Value) interface{} {
	*pointX = input[0].Float()
	*pointY = input[1].Float()
	*zoom = input[2].Float()
	...
	p := document.Call("createElement", "img")
	p.Set("src", " data:image/png;base64,"+base64.StdEncoding.EncodeToString(buf.Bytes()))
